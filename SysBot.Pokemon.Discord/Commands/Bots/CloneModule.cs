...
public class CloneModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public async Task CloneAsync(int code)
    {
        ...
        _ = QueueHelper<T>.AddToQueueAsync(Context, code, Context.User.Username, sig, new T(), PokeRoutineType.Clone, PokeTradeType.Clone, Context.User, 1, 1, false, false, lgcode: lgcode);
    ...
    public async Task CloneAsync([Summary("Trade Code")][Remainder] string code)
    {
        ...
        // Add to queue asynchronously
        _ = QueueHelper<T>.AddToQueueAsync(Context, tradeCode == 0 ? Info.GetRandomTradeCode(userID) : tradeCode, Context.User.Username, sig, new T(), PokeRoutineType.Clone, PokeTradeType.Clone, Context.User, 1, 1, false, false, lgcode: lgcode);
