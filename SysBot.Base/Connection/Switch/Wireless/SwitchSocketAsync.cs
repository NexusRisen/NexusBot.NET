...
public sealed class SwitchSocketAsync : SwitchSocket, ISwitchConnectionAsync
{
    ...
    public async Task<byte[]> PixelPeek(CancellationToken token)
    {
        ...
        var bytes = await ReadRaw(SwitchCommand.PixelPeek(), 0x20000, token).ConfigureAwait(false); // Default max screenshot size
        return Decoder.ConvertHexByteStringToBytes(bytes);
    }
