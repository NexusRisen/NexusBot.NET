...
#pragma warning restore CS8604 // Possible null reference argument.
    ...
    private async Task<PA8> ApplyAutoOT(PA8 toSend, TradePartnerLA tradePartner, SAV8LA sav, CancellationToken token)
    {
        try
        {
            ...
            {
                Log("AutoOT: Trade partner info is null, skipping.");
                return toSend;
            }
            ...
            uint tidsid = uint.Parse(tradePartner.SID7) * 1_000_000 + uint.Parse(tradePartner.TID7);

            // Utilize PKHeX.Core.AutoMod logic for perfect memory and handler generation
            SysBot.Pokemon.Helpers.AutoOTHelper.ApplyAutoOT(cln, tradePartner.TrainerName, tradePartner.Gender, tradePartner.Language, tidsid & 0xFFFF, tidsid >> 16);
            ...
            if (tradela.Valid)
            {
                Log("Pokemon is valid, applying AutoOT (LA).");
                await SetBoxPokemonAbsolute(BoxStartOffset, cln, token, sav).ConfigureAwait(false);
                return cln;
            }
            else
            {
                Log($"Trade Pokemon can't have AutoOT applied (LA). Legality: {tradela.Report()}");
                return toSend;
            }
        }
        catch (Exception ex)
        {
            Log($"AutoOT: Unexpected error during application (LA): {ex.Message}");
            LogUtil.LogError($"AutoOT error (LA): {ex}", "AutoOT");
            return toSend;
        }
    }
